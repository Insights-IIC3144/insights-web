export const fmtMoney = (n: number, opts?: { compact?: boolean; decimals?: number }) =>
  opts?.compact
    ? new Intl.NumberFormat("en-US", { notation: "compact", maximumFractionDigits: 1, style: "currency", currency: "USD" }).format(n)
    : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: opts?.decimals ?? 0 }).format(n);
