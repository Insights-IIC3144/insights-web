...
export function CatalogTable({ products, loading }: CatalogTableProps) {
  ...
  const renderSortIcon = (field: SortField) => {
    ...
    return sortDirection === 'asc'
      ? <ChevronUp className="h-3.5 w-3.5 inline ml-1 text-black" />
      : <ChevronDown className="h-3.5 w-3.5 inline ml-1 text-black" />;
  };

  if (loading) {
    return (
      <Panel className="p-6">
        <div className="h-6 w-48 bg-panel-hover rounded animate-pulse mb-6" />
        <div className="space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-12 w-full bg-panel-hover/50 rounded animate-pulse" />
          ))}
        </div>
      </Panel>
    );
  ...
  return (
    <Panel className="flex flex-col">
      <div className="p-4 sm:px-6 sm:py-5 border-b border-panel-border">
        <h3 className="text-base font-semibold text-black">Rendimiento por Producto</h3>
        <p className="text-sm text-panel-muted mt-1">Desglose de ingresos, ventas y devoluciones de tu catálogo.</p>

        {/* Toolbar */}
        <div className="mt-5 flex flex-col sm:flex-row gap-3 justify-between">
          <div className="relative w-full sm:w-[450px]">
            <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-panel-muted" />
            <Input
          ...
          <div className="flex flex-1 items-center justify-end gap-2 overflow-x-auto">
            <Filter className="h-4 w-4 text-panel-muted hidden sm:block ml-2" />
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-panel-muted uppercase bg-panel-hover/30 border-b border-panel-border">
            <tr>
              <th className="px-6 py-3 font-medium">Producto</th>
              <th className="px-6 py-3 font-medium">Categoría</th>
              <th className="px-6 py-3 font-medium text-right">Precio</th>
              <th
                className="px-6 py-3 font-medium text-center cursor-pointer hover:text-black hover:bg-panel-hover/50 transition-colors select-none"
                onClick={() => handleSort('unitsSold')}
              >
                Unidades Vendidas {renderSortIcon('unitsSold')}
              </th>
              <th
                className="px-6 py-3 font-medium text-right cursor-pointer hover:text-black hover:bg-panel-hover/50 transition-colors select-none"
          ...
          <tbody className="divide-y divide-panel-border/50">
            {currentItems.map((product) => (
              <tr key={product.productId} className="hover:bg-panel-hover/30 transition-colors">
                <td className="px-6 py-4 font-medium text-black max-w-[200px] truncate" title={product.productName}>
                  {product.productName}
                </td>
                <td className="px-6 py-4 text-panel-foreground">{product.category}</td>
                <td className="px-6 py-4 text-right text-panel-foreground">{formatCurrency(product.retailPrice)}</td>
                <td className="px-6 py-4 text-center text-panel-foreground">{product.unitsSold.toLocaleString()}</td>
                <td className="px-6 py-4 text-right text-black font-medium">{formatCurrency(product.revenueNet)}</td>
            ...
            {currentItems.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-panel-muted">
      ...
      {processedProducts.length > 0 && (
        <div className="p-4 border-t border-panel-border flex items-center justify-between text-sm">
          <div className="text-panel-muted">
            Mostrando <span className="font-medium text-black">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span> a <span className="font-medium text-black">{Math.min(currentPage * ITEMS_PER_PAGE, processedProducts.length)}</span> de <span className="font-medium text-black">{processedProducts.length}</span> resultados
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-panel-muted px-2">
